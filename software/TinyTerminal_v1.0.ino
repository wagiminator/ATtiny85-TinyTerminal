...
// clear the top line, then scroll the display up by one line
...
// plot a character; line = 0 to 7; column = 0 to 20
void OLED_plotChar(char c, uint8_t line, uint8_t column) {
  column <<= 1;                           // calculate column
  column += column << 1;                  // -> column = column * 6;
  I2C_start(OLED_ADDR);                   // start transmission to OLED
  I2C_write(OLED_CMD_MODE);               // set command mode
  I2C_write(0xB0 + ((line + Scroll) & 0x07)); // set line
  I2C_write(0x00 + (column & 0x0F));      // set column start low
  I2C_write(0x10 + (column >> 4));        // set column start high
  I2C_stop();                             // stop transmission
  I2C_start(OLED_ADDR);                   // start transmission to OLED
  I2C_write(OLED_DAT_MODE);               // set data mode
...
// prints a character or handle control characters
void OLED_print(char c) {
  c = c & 0x7F; // Ignore top bit
  if (c >= 32) {
    OLED_plotChar(c, Line, Column++);
    if (Column > 20) {
      Column = 0;
